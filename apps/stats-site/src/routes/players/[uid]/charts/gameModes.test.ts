...
describe("getGameModeBreakdowns", () => {
    it("normalizes, labels, filters, and sorts game mode records", () => {
    ...
    it("creates an empty model for missing and zero-only data", () => {
        expect(getBreakdown("wins", { wins: null, losses: { m00: 0 } }).rows).toEqual([])
    })

    it("uses one stable color for each known game mode", () => {
