<script lang="ts">
    ...
    type Cell = "gameMode" | "isTeams" | "map" | "playerCount"

    const { server, cell }: { server: Server; cell: Cell } = $props()
        ...
        {#if mapImageUrl}
            <Popover.Root>
                <Popover.Trigger openOnHover openDelay={150} closeDelay={100}>
                    {#snippet child({ props })}
                        <A {...props} href={mapArticleUrl} external class="font-medium underline">
                            {MapName[server.map]}
                        </A>
                    {/snippet}
                </Popover.Trigger>
