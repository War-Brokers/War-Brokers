...
export default (tags: string[]) =>
    publicProcedure
        .meta({
            openapi: {
                method: "GET",
                path: "/players/percentile/timeAlive",
                description: "Retrieves percentile of player's time alive",
                tags,
            },
        })
        .input(z.object({ uid }))
        ...
        .query(async ({ input }) => {
            ...
            const res = await db.getPercentile("time_alive", uid)

            if (!res.success) {
                if (res.reason === FailReason.PlayerNotFound) throw PlayerNotFoundTRPCError(uid)

                console.error(`/players/percentile/timeAlive?uid=${uid} failed: ${res.reason}`)
